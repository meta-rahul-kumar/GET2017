 ...
 *
 */
public class InMemoryProductPromoDAO implements ProductPromoDAO{
	private static InMemoryProductPromoDAO productPromoDAO;
	HashMap<Integer,ProductPromo> totalProductPromos = new HashMap<>();

	/**
	 * FileProductPromoDAO private constructor
	 */
	private InMemoryProductPromoDAO() {
	 ...
	 * @return
	 */
	public static InMemoryProductPromoDAO getInstance() {
		if(productPromoDAO == null) {
			synchronized (InMemoryProductPromoDAO.class) {
				if(productPromoDAO == null) {
					productPromoDAO = new InMemoryProductPromoDAO();
				}
			}
		}
