...
public class Users {
	 ...
	 * @return
	 */
	public boolean authenticateUserByEmail(String email) {
		boolean exists = false;
		for (String key : totalUsers.keySet()) {
			if(email.equals(totalUsers.get(key).getUserEmail())) {
				exists = true;
			}
		}
		return exists;
	...
	public void showUsers() {
		...
		for (String key : totalUsers.keySet()) {
			System.out.println(totalUsers.get(key).getUserId() + "\t" + totalUsers.get(key).getUserName() + "\t" + totalUsers.get(key).getUserGender() + "\t" + totalUsers.get(key).getUserEmail());
		}
	}
	...
	public boolean addUser() {
		...
		for(String key : totalUsers.keySet()) {
			if(totalUsers.get(key).getUserEmail().equals(userEmail)) {
				exists = true;
			}
		}
		
		if(exists) {
			System.out.println("Email ID already Exixts");
		}else {
			User user = new User();
			user.setUser(max+"", userName, userGender, userEmail);
